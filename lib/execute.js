 ...
 */
const tmpFilename = function (extension) {
 ...
 * })
 */
const createTemporaryScriptFile = function (extension, contents, callback) {
 ...
 * });
 */
const executeScript = function (scriptPath, callback, times) {
  times = times || 5;

  childProcess.execFile(scriptPath, function (error, stdout, stderr) {
    if (error) {
 ...
 * });
 */
exports.extractAndRun = function (script, callback) {
  const extension = path.extname(script.originalFilename);

  createTemporaryScriptFile(extension, script.content, function (writeError, temporaryPath) {
    if (writeError) {
      return callback(writeError);
    }

    executeScript(temporaryPath, function (executeError, output) {
      // Attempt to clean up, but ignore failure
      fs.unlink(temporaryPath, function (unlinkError) {
        if (unlinkError) {
